...
public class DateUtil extends DateUtils{
	...
	public static int WeekOfYear(String time){
		Calendar cal=Calendar.getInstance();
		cal.setTime(parseDate(time));
		return  cal.get(Calendar.WEEK_OF_YEAR);

	}
	//算星期
	public static String CaculateWeekDay(String time)
	{
	...
	//算日期
	public static  String getDate(String time,int weeks, int weekday)//开学日期周次，星期
	{

		Calendar calendar = Calendar.getInstance();
//        Date starttime = new Date(n, y, r);//开学日期
		calendar.setTime(parseDate(time));
		int days;//根据周次，星期算出天数差
		days = (weeks - 1) * 7 + weekday;//天数差
		calendar.add(Calendar.DAY_OF_MONTH, days);//所求日期
	...
	//将大写的星期转化成数字
	public static int weekDays(String week)
	{
